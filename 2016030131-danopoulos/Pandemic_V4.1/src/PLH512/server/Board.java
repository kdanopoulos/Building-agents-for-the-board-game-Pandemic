...
public class Board implements Serializable,Cloneable{
	...
	public void addCubes2(String cityToAddCubes, int numberOfCubes, String cubeColor)
	{
		updateQuarantineSpecialistProtectionList();
		updateMedicProtectionList(cubeColor);
		if (underQuarantineSpecialistProtection.contains(cityToAddCubes) && !isInitianInfection) {
	...
	public void infectCities2(int numberOfCitiesToInfect, int amountOfCubesToAdd)
	{
		for(int j=0 ; j < numberOfCitiesToInfect ; j++)
		{
			if (gameEnded == false)
			{
				String cityToInfect = infectedDeck.get(0).toString();
				addCubes2(cityToInfect, amountOfCubesToAdd, colorOf(cityToInfect));				
				discardedPile.add(cityToInfect);
				infectedDeck.remove(0);
			}
		}
	...
	public void drawCards2(int playerID, int numberOfCardsToDraw)
	{	
		if (isLimitSeven) 
		{
			if ((getHandOf(playerID).size() + 2) > 7)
		...
		if (playersDeck.size() >= numberOfCardsToDraw)
		{
			for(int j=0 ; j < numberOfCardsToDraw ; j++)
			{
				...
				else
				{
					currentInfectionPointer = currentInfectionPointer + 1;				
					String cityToInfect = infectedDeck.get(infectedDeck.size()- 1).toString();			
					addCubes2(cityToInfect, 3, colorOf(cityToInfect));					
					discardedPile.add(cityToInfect);
					infectedDeck.remove(infectedDeck.size() - 1);					
					Collections.shuffle(discardedPile);
					infectedDeck.addAll(0, discardedPile);
	...
	public void manualRemoveResearchStation2(String cityName)
	{
		if (this.RSCount > 0){
			boolean foundCity = false;	
			for (City tmpCity : cityList){	
	...
	public boolean driveTo2(int playerID, String destination)
	{	
		City currentCity = searchForCity(getPawnsLocations(playerID));
		boolean isLegal = false;
		...
		if (isLegal) {
			setPawnsLocations2(playerID, destination);
			return true;
		}
		else {
			return false;
	...
	public boolean directFlight2(int playerID, String destination)
	{
		boolean isLegal = false;
	...
	public boolean charterFlight2(int playerID, String destination)
	{
		boolean isLegal = false;
	...
	public boolean shuttleFlight2(int playerID, String destination)
	{
		boolean isLegal = false;
	...
	public boolean buildRS(int playerID, String cityToBuild)
	{
		...
		boolean hasAlreadyResearchStation = this.searchForCity(cityToBuild).getHasReseachStation();
		boolean isLegal = false;
		
		if(getPawnsLocations(playerID).equals(cityToBuild) && isOperationsExpert && hasAlreadyResearchStation==false)
			isLegal = true;
		else if(getPawnsLocations(playerID).equals(cityToBuild) && getHandOf(playerID).contains(cityToBuild) && hasAlreadyResearchStation==false)
			isLegal = true;
		else 
			isLegal = false;
		...
		if (isLegal && isOperationsExpert) {
			...
			this.searchForCity(cityToBuild).setHasReseachStation(true);
	...
	public boolean buildRS2(int playerID, String cityToBuild)
	{
		boolean isOperationsExpert = getRoleOf(playerID).equals("Operations Expert");
		boolean hasAlreadyResearchStation = this.searchForCity(cityToBuild).getHasReseachStation();
		boolean isLegal = false;
	...
	public boolean removeRS2(int playerID, String cityToRemove)
	{
		
		if (getRSLocations().contains(cityToRemove))
		{
			manualRemoveResearchStation2(cityToRemove);
			return true;
		}
		else 
		{
	...
	public boolean treatDisease2(int playerID, String cityToTreat, String color)
	{
		City currentCity = searchForCity(getPawnsLocations(playerID));
		boolean isMedic = getRoleOf(playerID).equals("Medic");
		int cubesRemoved = 0;
	...
	public boolean cureDisease2(int playerID, String colorToCure)
	{
		boolean isScientist = getRoleOf(playerID).equals("Scientist");
		int cardsColorCount = 0;
		int cardsDiscarted = 0;
	...
	public boolean cureDisease2(int playerID, String colorToCure, String card1, String card2, String card3, String card4)
	{
		boolean isScientist = getRoleOf(playerID).equals("Scientist");
		int cardsColorCount = 0;
		boolean isLegal = false;
	...
	public void actionPass2(int playerID){
	}
...
public void checkMedicSpecialAbility2(int playerID){
		if (!getRoleOf(playerID).equals("Medic"))
			return;
		String currentCityString = getPawnsLocations(playerID);
		City currentCity = searchForCity(currentCityString);
	...
	public void operationsExpertTravel2(int playerID, String destination, String cardToThrow)
	{
		int operSpID = 5;
		String currentCiyString;
		boolean isLegal = false;
		...
		if (isLegal) {
			setPawnsLocations2(operSpID, destination);
			getHandOf(operSpID).remove(cardToThrow);
		}
	}
	...
	public void setPawnsLocations2(int playerID, String pawnLocation) {
		this.pawnsLocations[playerID] = pawnLocation;
		
		checkMedicSpecialAbility2(playerID);
	}
